...
public class NeoStore extends AbstractStore
{
    ...
    protected boolean versionFound( String version )
    {
        ...
        {
            ByteBuffer buf = ByteBuffer.wrap( new byte[9] );
            buf.put( Record.IN_USE.byteValue() ). putLong( 0 );
            buf.flip();
            try
            {
                getFileChannel().write( buf, 3 * RECORD_SIZE );
            }
            catch ( IOException e )
            {
        ...
        else if ( version.equals( "NeoStore v0.9.3" ) )
        {
